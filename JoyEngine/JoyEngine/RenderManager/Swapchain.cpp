...
namespace JoyEngine
{
	Swapchain::Swapchain()
	{
		...
		if (swapChainSupport.capabilities.maxImageCount > 0 && m_swapchainImageCount > swapChainSupport.capabilities.
			maxImageCount)
		{
	...
	Swapchain::~Swapchain()
	{
		...
		{
			vkDestroyImageView(JoyContext::Graphics->GetDevice(), m_swapChainImageViews[i],
			                   JoyContext::Graphics->GetAllocationCallbacks());
		}

		vkDestroySwapchainKHR(JoyContext::Graphics->GetDevice(), m_swapChain,
		                      JoyContext::Graphics->GetAllocationCallbacks());
	}
}
